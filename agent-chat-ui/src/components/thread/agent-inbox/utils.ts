...
export interface HumanConfirmationSummary {
  ...
  decisions: Record<DecisionType, boolean>;
}
...
function asSummaryText(value: unknown): string | undefined {
...
function asSummaryList(value: unknown): string[] {
  if (Array.isArray(value)) {
    return value.flatMap((item) => {
      const text = asSummaryText(item);
      return text ? [text] : [];
    });
  }

  const text = asSummaryText(value);
  return text ? [text] : [];
}

function firstNonEmptyList(...values: unknown[]): string[] {
  for (const value of values) {
    const list = asSummaryList(value);
    if (list.length > 0) {
      return list;
    }
  }
...
export function extractHumanConfirmationSummary(
  hitlRequest: HITLRequest,
): HumanConfirmationSummary {
  const actionRequest = hitlRequest.action_requests?.[0];
  const actionArgs = asRecord(actionRequest?.args);
  const metadata = asRecord(hitlRequest.metadata);
  const reviewConfig =
  ...
  return {
    description:
      asSummaryText(actionRequest?.description) ??
      asSummaryText(metadata.description) ??
      asSummaryText(actionArgs.description),
    risk_level:
      asSummaryText(metadata.risk_level) ??
      asSummaryText(actionArgs.risk_level),
    destructive_effects: firstNonEmptyList(
      metadata.destructive_effects,
      actionArgs.destructive_effects,
    ),
    data_sources: firstNonEmptyList(
      metadata.data_sources,
      actionArgs.data_sources,
    ),
    execution_mode:
      asSummaryText(metadata.execution_mode) ??
      asSummaryText(actionArgs.execution_mode),
    dry_run_preview: metadata.dry_run_preview ?? actionArgs.dry_run_preview,
    decisions: {
      approve: allowedDecisions.includes("approve"),
      edit: allowedDecisions.includes("edit"),
      reject: allowedDecisions.includes("reject"),
    },
  };
}
